...
if (!$foundConfig) {
	...
	while (empty($sitename)) {
		$sitename = 'test.localhostaspen';#readline("Enter the sitename to be setup (e.g., demo.localhost, library.production) > ");
	}
	...
	while (empty($variables['library'])) {
		$variables['library'] = 'test.localhostaspen';#readline("Enter the library or consortium name, e.g., Aspen Public Library > ");
	}

	$variables['title'] = '';
	while (empty($variables['title'])) {
		$variables['title'] = 'test.localhostaspen';#readline("Enter the title of the site, e.g., Aspen Demo (may be same as library name) > ");
	}

	$variables['url'] = '';
	while (empty($variables['url'])) {
		$variables['url'] = 'localhost:80';#readline("Enter the url where the site will be accessed, e.g., https://demo.aspendiscovery.org or http://demo.localhost > ");
	}
	...
	if (!$siteOnWindows) {
		...
		while (empty($linuxOS) || !in_array($linuxOS, $linuxArray)) {
			$linuxOS = 'debian';#readline("Enter the name of your Linux OS (e.g., ".implode (" / ", $linuxArray)." ) > ");
		}
	}

	$variables['solrPort'] = '8080';#readline("Which port should Solr run on (typically 8080)? ");
	if (empty($variables['solrPort'])){
		$variables['solrPort'] = "8080";
	}

	$variables['ils'] = 'Koha';#readline("Which ILS does the library use? (default is Koha) > ");
	...
	if ($variables['ils'] == 'Koha'){
		...
		while (empty($variables['ilsDBHost'])) {
			$variables['ilsDBHost'] = 'Koha';#readline("Database host for Koha > ");
		}
		$variables['ilsDBPort'] = '';
		while (empty($variables['ilsDBPort'])) {
			$variables['ilsDBPort'] = '3306';#readline("Database port for Koha > ");
		}
		$variables['ilsDBName'] = '';
		while (empty($variables['ilsDBName'])) {
			$variables['ilsDBName'] = 'Koha';#readline("Database schema name for Koha > ");
		}
		$variables['ilsDBUser'] = '';
		while (empty($variables['ilsDBUser'])) {
			$variables['ilsDBUser'] = 'koha';#readline("Database username for Koha > ");
		}
		$variables['ilsDBPwd'] = '';
		while (empty($variables['ilsDBPwd'])) {
			$variables['ilsDBPwd'] = 'koha';#readline("Database password for {$variables['ilsDBUser']} for Koha > ");
		}
		$variables['ilsDBTimezone'] = 'Europe/London';#readline("Database timezone for Koha (e.g., US/Central) > ");
		if (empty($variables['ilsDBTimezone'])){
			$variables['ilsDBTimezone'] = 'US/Central';
		}
		$variables['ilsClientId'] = 'apiclient';#readline("Client ID for Koha API > ");
		$variables['ilsClientSecret'] = 'apisecret';#readline("Client Secret for Koha API > ");
	}

	while (empty($variables['ilsDriver'])) {
		$variables['ilsDriver'] = 'koha';#readline("Enter the Aspen Driver for the ILS  > ");
	}

	$variables['ilsUrl'] = '';
	while (empty($variables['ilsUrl'])) {
		$variables['ilsUrl'] = 'localhost:8080';#readline("Enter the url of the OPAC for the ILS  > ");
	}

	//This can be blank
	$variables['staffUrl'] = 'localhost:8081';#readline("Enter the url of the staff client for the ILS  > ");

	$variables['aspenDBName'] =  'aspen';#readline("Database name for Aspen (default: aspen) > ");
	if (empty($variables['aspenDBName'])){
		$variables['aspenDBName'] = "aspen";
	}

	$variables['aspenDBUser'] =  'aspensuper';#readline("Database username for Aspen (default: root) > ");
	...
	while (empty($variables['aspenDBPwd'])) {
		$variables['aspenDBPwd'] = 'aspensuper';#readline("Database password for {$variables['aspenDBUser']} for Aspen > ");
	}

	$variables['timezone'] =  'Europe/London';#readline("Enter the timezone of the library (e.g. America/Los_Angeles, check http://www.php.net/manual/en/timezones.php) > ");
	...
	while (empty($variables['aspenAdminPwd'])) {
		$variables['aspenAdminPwd'] = 'password';#readline("Select a password for the 'aspen_admin' user > ");
	}
}
